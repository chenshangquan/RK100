...
CGifImage::CGifImage(IStream *pStream)
{
	if ( pStream != NULL)
	{
		m_pIstream = pStream;
...
BOOL CGifImage::SaveAsFile(LPCTSTR pszFileName)
{
	...
	CLSID clsid = GetEncoderClsidByExtension( CT2CW( lpExtension ) );
	if (CLSID_NULL == clsid)
		return FALSE;

	Gdiplus::Status status = m_pImage->Save( CT2CW( pszFileName ), &clsid, NULL);
