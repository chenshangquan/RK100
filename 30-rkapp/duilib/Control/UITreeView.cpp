...
namespace DuiLib
{
	...
	void CTreeViewUI::SetItemExpand( bool _Expanded,CTreeNodeUI* _TreeNode /*= NULL*/ )
	{
		if(_TreeNode)
		{
			//从外部控制展开后FolderButton没有同步 by zh. 20130830
			CCheckBoxUI *pFolder = _TreeNode->GetFolderButton();
			if (pFolder && pFolder->IsVisible() && pFolder->GetCheck() == _Expanded)
			{
				pFolder->SetCheck(!_Expanded);
			}
