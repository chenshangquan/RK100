...
bool CListElementUI::Select(bool bSelect)
{
    ...
    //if( bSelect == m_bSelected ) return true; //选中同一个时，依然响应SelectItem -2015.4.2 xcr
    m_bSelected = bSelect;
...
void CListElementUI::SetGdiPlus( bool bGdiPlus )
{
...
void CListLabelElementUI::DoEvent(TEventUI& event)
{
    ...
    {
        if( IsEnabled() ) {
            m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMDOWN);
            Select();
            Invalidate();
        }
        return;
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if( pInfo->bShowHtml )
		...
		CRenderEngine::DrawTextByGdiPlus(hDC, m_pManager, rcText, m_sText, iTextColor, \
