...
namespace DuiLib
{
    ...
    TFontInfo*  CLabelUI::GetFontInfo() const
    {      
        return m_pManager->GetFontInfo( GetFont() );
    }
	...
	void CLabelUI::SetTextPadding(RECT rc)
	{
		...
		if ( CPaintManagerUI::IsSelfAdaption() )
		{
			...
			m_rcTextPadding.bottom = m_rcTextPadding.bottom * fAdpY;
			m_rcTextPadding.top = m_rcTextPadding.top * fAdpY;
			m_rcTextPadding.left = m_rcTextPadding.left * fAdpX;
			m_rcTextPadding.right = m_rcTextPadding.right * fAdpX;
		}
	...
	bool CLabelUI::IsGdiPlus()
	{
		return m_bGdiPlus;
	}

	void CLabelUI::SetGdiPlus(bool bGdiPlus)
	{
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("align")) == 0 ) {
			...
			if( _tcsstr(pstrValue, _T("center")) != NULL ) {
				m_uTextStyle &= ~(DT_LEFT | DT_RIGHT | DT_SINGLELINE);
            ...
            if( _tcsstr(pstrValue, _T("singleline")) != NULL ) {
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		if(!GetEnabledEffect())
		{
			...
			if( IsEnabled() ) {
				...
				if( m_bShowHtml )
					...
					CRenderEngine::DrawTextByGdiPlus(hDC, m_pManager, rc, m_sText, m_dwDisabledTextColor, \
					m_iFont, DT_SINGLELINE | m_uTextStyle);
				else
		...
		else
		{
			...
			if(GetEnabledStroke() && GetStrokeColor() > 0)
			{
				...
				nRc.Offset(-1,0);
				nGraphics.DrawString( CT2CW( m_TextValue.GetData() ), -1,&nFont,nRc,&format,&nLineGrBrushStroke);
				nRc.Offset(2,0);
				nGraphics.DrawString( CT2CW( m_TextValue.GetData() ), -1,&nFont,nRc,&format,&nLineGrBrushStroke);
				nRc.Offset(-1,-1);
				nGraphics.DrawString( CT2CW( m_TextValue.GetData() ), -1,&nFont,nRc,&format,&nLineGrBrushStroke);
			...
			if(GetEnabledShadow() && (GetTextShadowColorA() > 0 || GetTextShadowColorB() > 0))
				nGraphics.DrawString( CT2CW( m_TextValue.GetData() ), -1,&nFont,nShadowRc,&format,&nLineGrBrushA);

			nGraphics.DrawString( CT2CW( m_TextValue.GetData() ), -1,&nFont,nRc,&format,&nLineGrBrushB);
