...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if ( event.Type == UIEVENT_GESTURENOTIFY )  //nty消息和ldow nty先来 dyy 2015-6-15
		{
			if( ::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled() ) 
			{ 
				if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_BUTTONDOWN);//调节焦距等要WM_LBUTTONDOWN消息调节 dyy 2015-10-9 
				m_uButtonState |= UISTATE_PUSHED | UISTATE_CAPTURED;
				Invalidate();

// 				TCHAR chTemp[128];
// 				wsprintf(chTemp,_T("UIEVENT_GESTURENOTIFY  PtInRect\n" ));
// 				::OutputDebugString(chTemp);
			}

			return;
		}
		if( event.Type == UIEVENT_BUTTONDOWN )//调节焦距等要WM_LBUTTONDOWN消息调节 dyy 2015-3-9 
		{
			if( ::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled() ) {

				if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_BUTTONDOWN);
				...
				if (m_bPassMoveNty)
				{
					event.pSender->GetParent()->DoEvent(event);
				}
// 				TCHAR chTemp[128];
// 				wsprintf(chTemp,_T("UIEVENT_BUTTONDOWN  PtInRect\n" ));
// 				::OutputDebugString(chTemp);
			}
			return;
		}
		...
		if( event.Type == UIEVENT_GESTUREUP )
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
				m_uButtonState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
				Invalidate();
...
// 				TCHAR chTemp[128];
// 				wsprintf(chTemp,_T("UIEVENT_GESTUREUP  PtInRect\n" ));
// 				::OutputDebugString(chTemp);
			}
...
// 				TCHAR chTemp[128];
// 				wsprintf(chTemp,_T("UIEVENT_BUTTONUP  PtInRect\n" ));
// 				::OutputDebugString(chTemp);
			}
	...
	void CButtonUI::SetPassMoveNty(bool bPassMoveNty)
	{
		m_bPassMoveNty = bPassMoveNty;
	}

	bool CButtonUI::IsPassMoveNty()
	{
		return m_bPassMoveNty;
	}
	...
	void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("passmove")) == 0 ) SetPassMoveNty(_tcscmp(pstrValue, _T("true")) == 0);
	...
	void CButtonUI::PaintText(HDC hDC)
	{
		...
		if( m_bShowHtml )
			...
			CRenderEngine::DrawTextByGdiPlus(hDC, m_pManager, rc, m_sText, clrColor, \
