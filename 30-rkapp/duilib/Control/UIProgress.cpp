...
namespace DuiLib
{
	CProgressUI::CProgressUI() : m_bHorizontal(true), m_nMin(0), m_nMax(100), m_nValue(0), m_bStretchForeImage(true),m_dwForeBkColor(0)
	{
	...
	void CProgressUI::SetForeColor(DWORD dwForeBkColor)
	{
		if ( m_dwForeBkColor == dwForeBkColor) return;
		
		m_dwForeBkColor = dwForeBkColor;
		Invalidate();
	}
	...
	void CProgressUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("forebkcolor")) == 0 )
		{
	...
	void CProgressUI::PaintStatusImage(HDC hDC)
	{
		...
		else if(m_dwForeBkColor != 0) {
			RECT rcCurProgress = m_rcPaint;
			rcCurProgress.right = m_rcPaint.left + (rc.right - rc.left);
			CRenderEngine::DrawColor(hDC, rcCurProgress, GetAdjustColor(m_dwForeBkColor));
			return;
		}
	}
