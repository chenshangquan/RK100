                        ...
                        int nSrcX, int nSrcY, int wSrc, int hSrc, BLENDFUNCTION ftn)
{
    ...
    //Creates Source DIB
	BITMAPINFO bmiSrc;
	ZeroMemory(&bmiSrc, sizeof(bmiSrc));

	bmiSrc.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
    bmiSrc.bmiHeader.biWidth = dwWidth;
    bmiSrc.bmiHeader.biHeight = dwHeight;
    bmiSrc.bmiHeader.biPlanes = 1;
    bmiSrc.bmiHeader.biBitCount = 32;
    bmiSrc.bmiHeader.biCompression = BI_RGB;
    bmiSrc.bmiHeader.biSizeImage = dwWidth * dwHeight;
    ...
    HBITMAP hSrcDib = CreateDIBSection (
        hSrcDC, &bmiSrc, DIB_RGB_COLORS, (void **)&pSrcBits,
        NULL, NULL);
	...
	BITMAPINFO bmiDest;
	ZeroMemory(&bmiDest, sizeof(bmiDest));

    bmiDest.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
    bmiDest.bmiHeader.biWidth = dwWidth;
    bmiDest.bmiHeader.biHeight = dwHeight;
    bmiDest.bmiHeader.biPlanes = 1;
    bmiDest.bmiHeader.biBitCount = 32;
    bmiDest.bmiHeader.biCompression = BI_RGB;
    bmiDest.bmiHeader.biSizeImage = dwWidth * dwHeight;
    bmiDest.bmiHeader.biXPelsPerMeter = 0;
    bmiDest.bmiHeader.biYPelsPerMeter = 0;
    bmiDest.bmiHeader.biClrUsed = 0;
    bmiDest.bmiHeader.biClrImportant = 0;

    COLORREF* pDestBits = NULL;
    HBITMAP hDestDib = CreateDIBSection (
        hDC, &bmiDest, DIB_RGB_COLORS, (void **)&pDestBits,
        NULL, NULL);
...
CGifImage* CRenderEngine::LoadGif( STRINGorID bitmap, LPCTSTR type, DWORD mask )
{
	LPBYTE pData = NULL;
	DWORD dwSize = 0;
	...
	hImageMemory = GlobalAlloc(GMEM_MOVEABLE, dwSize);                    //分配堆  
	void* pImageMemory = GlobalLock(hImageMemory);                            //锁定堆，并且得到堆指针  
	memcpy( pImageMemory, pData, dwSize );
	delete[] pData;
	IStream* pIStream = NULL;  
	CreateStreamOnHGlobal(hImageMemory, TRUE, &pIStream);        //得到IStream  
	return new CGifImage(pIStream);                           
}
...
bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
		...
		bool bHole, bool bTiledX, bool bTiledY, bool bIsAlwaysChangeBk)
{
	...
	if( sImageResType.IsEmpty() ) {
		data = pManager->GetImageEx((LPCTSTR)sImageName, NULL, dwMask, bIsAlwaysChangeBk);
	}
	else {
		data = pManager->GetImageEx((LPCTSTR)sImageName, (LPCTSTR)sImageResType, dwMask, bIsAlwaysChangeBk);
	}
	...
	if (rcBmpPart.top == 0) //以图像大小定位置时 top不是0 此时bottom不变 
	{
...
bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
                                          LPCTSTR pStrImage, LPCTSTR pStrModify, bool bIsAlwaysChangeBk/* = false*/)
{
    ...
    for( int i = 0; i < 2; ++i,image_count = 0 ) {
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
							rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY, false);
                ...
                else if( sItem == _T("dest") )/*xml使用，需要进行自适应*/ {
					RECT rcItemstr = {0};//字符串坐标数值
					rcItemstr.left = _tcstol(sValue.GetData(), &pstr, 10); ASSERT(pstr);
					rcItemstr.top = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
					rcItemstr.right = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
					rcItemstr.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
					//自适应需调节背景图位置坐标pos
					if ( CPaintManagerUI::IsSelfAdaption() )
					{
						float fAdpX, fAdpY;
						CPaintManagerUI::GetAdpResolution( &fAdpX, &fAdpY );
						rcItemstr.left *= fAdpX;
						rcItemstr.top *= fAdpY;
						rcItemstr.right *= fAdpX;
						rcItemstr.bottom *= fAdpY;
					}
					//确定比例缩放后的位置
                    rcItem.left = rc.left + rcItemstr.left;   
                    rcItem.top = rc.top + rcItemstr.top;
                    rcItem.right = rc.left +rcItemstr.right;
					if (rcItem.right > rc.right) rcItem.right = rc.right;
                    rcItem.bottom = rc.top + rcItemstr.bottom;
					if (rcItem.bottom > rc.bottom) rcItem.bottom = rc.bottom;
                }
				else if( sItem == _T("Dest") ) /*库代码内使用，不需要进行自适应*/{
	...
	DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
		rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY, bIsAlwaysChangeBk);
...
// 转换GDI字体风格到GDI+格式 by zjl 2013/11/01
void StyleToStringFormat( StringFormat& format, UINT uStyle )
{
	if ( uStyle & DT_CENTER ) format.SetAlignment( StringAlignmentCenter );
	if ( uStyle & DT_RIGHT ) format.SetAlignment( StringAlignmentNear );
	if ( uStyle & DT_LEFT ) format.SetAlignment( StringAlignmentFar );
	if ( uStyle & DT_SINGLELINE ) format.SetFormatFlags( StringFormatFlagsNoWrap );
	if ( uStyle & DT_RTLREADING ) format.SetFormatFlags( StringFormatFlagsDirectionRightToLeft );
	if ( uStyle & DT_NOCLIP ) format.SetFormatFlags( StringFormatFlagsNoClip );
	if ( uStyle & DT_TOP ) format.SetLineAlignment( StringAlignmentNear );
	if ( uStyle & DT_VCENTER ) format.SetLineAlignment( StringAlignmentCenter );
	if ( uStyle & DT_BOTTOM ) format.SetLineAlignment( StringAlignmentFar );
	if ( uStyle & DT_END_ELLIPSIS) format.SetTrimming( StringTrimmingEllipsisCharacter );
	if ( uStyle & DT_PATH_ELLIPSIS ) format.SetTrimming( StringTrimmingEllipsisPath );
	if ( uStyle & DT_WORD_ELLIPSIS ) format.SetTrimming( StringTrimmingEllipsisWord );
	if ( uStyle & DT_NOPREFIX ) format.SetHotkeyPrefix( HotkeyPrefixNone );
	if ( uStyle & DT_HIDEPREFIX ) format.SetHotkeyPrefix( HotkeyPrefixHide );
	//format.SetFormatFlags( uStyle );
}

// 使用GDI+绘制文字，支持透明通道 by zjl 2013/11/01
void CRenderEngine::DrawTextByGdiPlus(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	...
	// 创建字体，绘制矩形，画刷
	RectF layoutRect( static_cast<REAL>( 0 ), static_cast<REAL>( 0 ),
	...
	StringFormat format;
	// 转换GDI字体属性到GDI+属性
	StyleToStringFormat( format, uStyle );
	...
	if ( uStyle & DT_CALCRECT )
	{
		...
		graphics.MeasureString( CT2CW( pstrText ), -1, &font, layoutRect, &format, &rcOut );
		rc.left = static_cast<LONG>( rcOut.GetLeft() );
		rc.right = static_cast<LONG>( rcOut.GetRight() );
		rc.top = static_cast<LONG>( rcOut.GetTop() );
		rc.bottom = static_cast<LONG>( rcOut.GetBottom() );
	}
	else
	{
		...
		if ( hBitmap == NULL )
		{
			...
			// 选中Bitmap
			HGDIOBJ hSave = ::SelectObject( hOff, hBitmap );
			...
			lpAlphaBlend(hDC, rc.left, rc.top, width, height, hOff, 0, 0, width, height, bf);

			::SelectObject( hOff, hSave	);
			::DeleteObject( hBitmap );
			::DeleteDC(hOff);			
	}
}
...
bool CRenderEngine::DrawImageGDIPlus( HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
	SIZE sz, Image* pImg, CPoint ptInCtl, RECT rcItem )/*const TImageInfo* imgInfo*/
{
	...
	if ( NULL != pImg )
	{
		int xPos = rcItem.left;
		int yPos = rcItem.top;

		int nImgWidth = sz.cx;/*imgInfo*/
		int nImgHeight = sz.cy;/*imgInfo*/

		graphics.DrawImage( pImg, xPos + ptInCtl.x, yPos + ptInCtl.y,nImgWidth, nImgHeight);
	}
